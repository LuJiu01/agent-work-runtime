...
- 代价是会话开始多一次存储往返（实测同区对象存储 200ms 上下）。**整个取回有一个 10 秒预算**：
  它在自己的工作线程上跑，超时就不再等（进程退出时该线程随之结束），只在上下文里留一句
  "left for later, run `awr workspace sync`"。之所以需要它，是因为存储自己的请求超时是 60s——
  对盯着看的操作者合理，对会话入口不合理。已经取到的文件留在磁盘上，下一次 `sync` 接着取。
...
写到 `<项目根>/.awr/workspace-credentials.json`，权限 600（不是 600 会在读取时被拒绝）。
...
- **提交保证**：R2/S3/MinIO 用 `If-Match` 真 CAS（`commit_mode: "cas"`）；
  ...
  每次提交多一次 HEAD 请求，把对端能钻进来的窗口从"整次发布"收窄到"一个请求"；
  仍是一次原子提交，缺的是互斥，所以在这个窗口里抢先提交的对端会被检测到并重读重试，
  而不是被覆盖。剩下的一请求窗口内的极端交错不保证不发生，按"接受 + 自愈"处理：
  被挤掉的条目在失主那边报成 `index_missing_local`，重新 `publish` 即可补回（内容对象仍在）。
